...
# ─── کشِ ساختارِ ثابتِ منوها (PANEL_CATEGORIES) که از یه هاست سلفِ آنلاین
# گرفته می‌شه؛ چون این ساختار برای همه‌ی کاربرها یکسانه، فقط یه‌بار (و بعد
# هر چند دقیقه برای رفرش) گرفته می‌شه، نه هر کلیک ───────────────────────────
...
def _panel_categories() -> dict:
    return (_categories_data or {}).get("categories", {})


def build_category_menu():
    cats = _panel_categories()
    order = (_categories_data or {}).get("order", [])
    return [
        (key, cats[key]["title"], cats[key].get("menu_style", "primary"))
        for key in order if key in cats
    ]
...
async def start_helper_bot():
    ...
    async with _helper_start_lock:
        ...
        # bot.py دیگه روی این هاست نیست (رفته روی هاست‌های سلف)؛ به‌جاش از
        # طریق panel_client (HTTP) با هاست سلفِ صاحبِ هر کاربر صحبت می‌کنیم.
    ...
    def _back_target(category_key, owner_tg_id):
        ...
        cat = _panel_categories().get(category_key, {})
        parent = cat.get("parent")
        if parent:
            return f"panel_cat_{parent}_{owner_tg_id}"
        return f"panel_menu_{owner_tg_id}"

    async def _category_buttons(owner_id, owner_tg_id, category_key, page=0):
        """دکمه‌های سطح ۲ (آیتم‌های داخل یک دسته) + دکمه‌های زیرمنو (children)
        + بازگشت، همه با پسوند مالک."""
        cat = _panel_categories().get(category_key, {})
        items = await asyncio.to_thread(panel_client.get_category_commands, owner_id, category_key) or []
    ...
    async def on_inline(event):
        owner_id, _account = bot_manager.get_owner_by_tg_id(event.query.user_id)
        if owner_id is None or not bot_manager.is_running(owner_id):
            await event.answer(
        ...
        if query_text == "جوین":
            join_msg, fj_channels = await asyncio.to_thread(panel_client.force_join_info, owner_id)
            join_msg = join_msg or "⛔ برای ارسال پیام ابتدا باید در کانال‌های زیر عضو شوید."
            fj_channels = fj_channels or []
            rows = []
    ...
    async def _handle_panel_callback(event):
        ...
        owner_id, _account = bot_manager.get_owner_by_tg_id(event.sender_id)
        if owner_id is None or not bot_manager.is_running(owner_id):
        ...
        if body.startswith("panel_cat_"):
            ...
            if direct is not None:
                if direct.startswith("PANELINFO::"):
                    ...
                    await asyncio.to_thread(panel_client.execute, owner_id, direct)
                return
            await event.edit(
                _category_text(cat["title"]),
                buttons=await _category_buttons(owner_id, owner_tg_id, category_key, page=0),
            )
            return
        ...
        if body.startswith("panel_item_page_"):
            ...
            await event.edit(
                _category_text(cat["title"]),
                buttons=await _category_buttons(owner_id, owner_tg_id, category_key, page=int(page_str)),
            )
            return
        ...
        if body.startswith("panel_item_"):
            ...
            await asyncio.to_thread(panel_client.execute, owner_id, command_text)
            ...
            try:
                await event.edit(
                    _category_text(cat["title"]),
                    buttons=await _category_buttons(owner_id, owner_tg_id, category_key, page=page),
                )
            except Exception:
                pass
            return
